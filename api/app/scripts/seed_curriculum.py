...
def _generate_topic(sub_strand_name: str, term: int, seq: int) -> str:
    """Return a plausible CBC topic title for a sub-strand/term/sequence."""
    prefixes = {
        1: ["Introduction to", "Getting started with", "Exploring", "Understanding the basics of"],
        2: ["Developing skills in", "Building on", "Advancing through", "Deepening knowledge of"],
        3: ["Mastering", "Consolidating", "Reviewing and applying", "Extending"],
    }
    return f"{prefixes[term][seq - 1]} {sub_strand_name}"


def _generate_outcomes(topic: str) -> list[str]:
    return [f"Explain key concepts related to {topic}", f"Apply skills learned in {topic} to real-life situations"]


def _generate_experiences(topic: str) -> list[str]:
    return [f"Participate in a guided activity on {topic}", f"Work in groups to solve problems related to {topic}"]


def _generate_questions(topic: str) -> list[str]:
    return [f"Why is {topic} important in daily life?", f"How can we use {topic} to solve community problems?"]


def _sample_lesson_content() -> list[dict]:
    """Curated content-bank entries to exercise schemes of work.

    Generates sample entries for every sub-strand in the curriculum
    across all three terms so teachers can generate schemes immediately
    without first populating the content bank manually.
    """
    catalogue = _load_catalogue_json()
    entries: list[dict] = []
    sub_strands = {row["code"]: row["name"] for row in catalogue["sub_strands"]}

    for code, name in sub_strands.items():
        for term in (1, 2, 3):
            for seq in (1, 2):
                topic = _generate_topic(name, term, seq)
                entries.append({
                    "sub_strand_code": code,
                    "term_number": term,
                    "sequence_order": seq,
                    "topic": topic,
                    "learning_outcomes": _generate_outcomes(topic),
                    "learning_experiences": _generate_experiences(topic),
                    "key_inquiry_questions": _generate_questions(topic),
                    "resources": [f"Coursebook pg. {term * 10 + seq}", "Teacher guide"],
                    "assessment_methods": ["Written exercise", "Oral questioning"],
                    "value_signs": ["Responsibility", "Co-operation"],
                    "core_competences": ["Critical thinking", "Communication and collaboration"],
                })
    return entries
